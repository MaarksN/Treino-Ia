...
const baseMigration = readFileSync(
  resolve(process.cwd(), 'supabase/migrations/20260511013442_user_periodization_plans.sql'),
  'utf-8',
);
const hardeningMigration = readFileSync(
  resolve(process.cwd(), 'supabase/migrations/20260513044000_harden_periodization_rls_user_id.sql'),
  'utf-8',
);
...
describe('supabase critical rls tables', () => {
  ...
  it('keeps hardening migration idempotent for user_id and policies', () => {
    ...
    expect(hardeningMigration).toContain(
      'drop policy if exists user_periodization_plans_select_own',
    );
    expect(hardeningMigration).toContain(
