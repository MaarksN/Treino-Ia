...
describe('critical API payload limits', () => {
  ...
  it.each([
    [
      ...
      '../api/jobs/create',
      '/api/jobs/create',
      { jobType: 'pdf_export', payload: { padding: 'x'.repeat(61_000) } },
    ],
    [
      'offline sync',
      '../api/sync/offline-actions',
      '/api/sync/offline-actions',
      { id: 'offline-1', type: 'workout.completed', payload: { padding: 'x'.repeat(121_000) } },
    ],
    [
      'health sync',
      '../api/health/sync',
      '/api/health/sync',
      { provider: 'apple_health', summary: { padding: 'x'.repeat(81_000) } },
    ],
    [
      'health OAuth start',
      '../api/health/oauth/start',
      '/api/health/oauth/start',
      { provider: 'strava', redirectTo: '/', padding: 'x'.repeat(8_500) },
    ],
  ])(
    'rejects oversized %s bodies before database writes',
    async (_name, modulePath, route, body) => {
  ...
  it('queues supported background jobs with a sanitized payload object', async () => {
    ...
    const response = await handler(
      new Request('http://localhost/api/jobs/create', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
  ...
  it('persists offline actions idempotently for the authenticated user', async () => {
    ...
    const response = await handler(
      new Request('http://localhost/api/sync/offline-actions', {
        method: 'POST',
        headers: {
