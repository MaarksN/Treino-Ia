...
- O que falhou: format check, preflight real, smoke sprint3, Lighthouse CI.
...
| Unit tests | `npm run test` | 196 files, 770 tests | PASS |
| Coverage | `npm run test:coverage` | 34.05/28.78/33.5/34.32 | PASS tecnico/PARTIAL |
...
| Format | `npm run format:check` | 717 arquivos | FAIL |
| Lighthouse | `@lhci/cli autorun` | `NO_FCP` | FAIL/BLOCKED |
...
- Corrigir format check ou criar PR dedicado de formatacao.
- Reexecutar Lighthouse ou corrigir causa de `NO_FCP`.
...
| LHCI NO_FCP | Media | debug/retry com preview server | Pendente |
...
| 7 | Corrigir format check | P2 | M | format PASS |
