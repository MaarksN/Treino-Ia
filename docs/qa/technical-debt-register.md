...
| DT-008 | 01/02    | arquitetura/codigo | Clones abaixo da meta; exports aparentemente nao usados ainda em triagem | P3  | BAIXA   |  7.7 | M       | NAO      |
...
Evidencia: `jscpd --min-lines 10 --reporters console src api` reduziu de 100 para 23 clones apos extrair helpers dos registries dos blocos, API, testes, acessibilidade e Gemini; duplicacao total ficou em 0.46% de linhas. `ts-prune --project tsconfig.json` ainda lista exports suspeitos para triagem manual.

Impacto: ruido estrutural de clones ficou abaixo da meta operacional `<30`; a triagem de exports segue como limpeza incremental.

Acao: manter `jscpd <30` como gate de regressao e triagem manual dos exports suspeitos.
