...
export function calculateWorkoutTonnage(draft: ActiveExerciseDraft[]): WorkoutTonnageSummary {
  const totalTonnage = draft.reduce(
    (sum, exercise) => sum + calculateExerciseVolume(exercise.sets),
    0,
  );
  const completedTonnage = draft.reduce(
    (sum, exercise) => sum + calculateExerciseVolume(exercise.sets.filter((set) => set.completed)),
    0,
  );
...
export function getRpeGuidance(rpeInput: string | number | null | undefined): RpeGuidance {
  ...
  if (rpe >= 9)
    return { label: 'RPE 9', guidance: 'Muito difícil, cerca de 1 repetição em reserva.' };
  ...
  if (rpe >= 6)
    return { label: 'RPE 6', guidance: 'Leve a moderado, várias repetições em reserva.' };
...
export function calculateRpeFromRir(rirInput: string | number): number {
  const rir = typeof rirInput === 'string' ? Number(rirInput.replace(',', '.')) : rirInput;
...
function getExerciseLogsByRecency(
  exerciseId: string,
...
export function detectSimplePlateau(exerciseId: string, history: WorkoutSession[]): PlateauSignal {
  ...
  const [latest, previous, oldest] = logs.map((log) =>
    calculateExerciseVolume(
      (log.sets ?? []).map((set) => ({ weight: set.weight, reps: set.reps })),
    ),
  );
...
export function suggestInitialExerciseDraft(
  exerciseId: string,
  history: WorkoutSession[],
): ExerciseAutofillSuggestion {
  const last = getExerciseLogsByRecency(exerciseId, history).find(
    (ex) => Boolean(ex.sets?.length) || Boolean(ex.actualWeight || ex.actualReps || ex.rpe),
  );
