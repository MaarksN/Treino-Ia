...
export function TrainingReportPanel({ history }: TrainingReportPanelProps) {
  ...
  return (
    <section
      ...
      className="rounded-[28px] border-2 border-brand-neon bg-brand-gray/80 p-6 shadow-brutal-neon md:p-8 scroll-mt-24"
    >
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <ReportMetric
          icon={<CalendarDays />}
          label="Sessoes"
          value={String(report.sessions).padStart(2, '0')}
        />
        <ReportMetric
          icon={<BarChart3 />}
          label="Volume"
          value={`${Math.round(report.totalVolume).toLocaleString('pt-BR')} kg`}
        />
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-[0.85fr_1.15fr]">
        <article className="rounded-[22px] border border-brand-light/10 bg-brand-dark p-5">
          ...
          <p className="mt-2 font-display text-5xl uppercase text-brand-light">
            {report.totalDurationMinutes} min
          </p>
          <p className="mt-2 font-mono text-xs leading-6 text-brand-light/65">
            Media de {report.averageDurationMinutes} min por treino em {report.activeDays} dia(s)
            ativo(s).
          </p>
        </article>
...
function ReportMetric({ icon, label, value }: { icon: ReactNode; label: string; value: string }) {
  return (
    <article className="rounded-[22px] border border-brand-light/10 bg-brand-dark p-4 transition-transform hover:-translate-y-0.5">
      ...
      <p className="mt-2 break-words font-display text-3xl uppercase leading-none text-brand-light">
        {value}
      </p>
    </article>
  );
