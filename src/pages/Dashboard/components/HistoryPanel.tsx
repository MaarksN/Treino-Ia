...
export function HistoryPanel({ history }: { history: WorkoutSession[] }) {
  return (
    <section
      ...
      className="mb-8 rounded-[28px] border-2 border-brand-light/15 bg-brand-gray/80 p-6 shadow-brutal-light md:p-8 scroll-mt-24"
    >
      ...
      {history.length === 0 ? (
        ...
        <div className="space-y-3">
          {history.slice(0, 5).map((session) => (
            <article
              key={session.id}
              className="rounded-[22px] border-2 border-brand-light/10 bg-brand-dark p-4 transition-all duration-200 hover:-translate-y-0.5 hover:border-brand-neon/70"
            >
              <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                <div>
                  <p className="font-display text-3xl uppercase text-brand-light">
                    {session.focus}
                  </p>
                  <p className="font-mono text-xs uppercase tracking-widest text-brand-muted">
                    {formatDate(session.completedAt)} | {session.completedExercises}/
                    {session.totalExercises} exercícios | {Math.round(session.totalVolume)} kg
                  </p>
                </div>
              ...
              {session.nextRecommendation && (
                <p className="mt-3 font-mono text-xs leading-6 text-brand-light/65">
                  {session.nextRecommendation}
                </p>
              )}
            </article>
