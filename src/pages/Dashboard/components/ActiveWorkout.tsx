    ...
    >
      <div className="mx-auto max-w-5xl">
        ...
        <section className="mb-6 rounded-[24px] border border-brand-light/15 bg-brand-dark/40 p-4 font-mono text-xs uppercase tracking-wider text-brand-light/85">
          <div className="grid gap-2 md:grid-cols-4">
            <p>
              Progresso: {summary.progress.completedExercises}/{summary.progress.totalExercises} (
              {summary.progress.percent}%)
            </p>
            <p>
              Tonelagem estimada: {Math.round(summary.tonnage.totalTonnage).toLocaleString('pt-BR')}{' '}
              kg
            </p>
            <p>
              Tonelagem concluída:{' '}
              {Math.round(summary.tonnage.completedTonnage).toLocaleString('pt-BR')} kg
            </p>
        ...
        >
          ...
          <p
            className="flex-1 text-center font-mono text-xs uppercase tracking-widest text-brand-light"
            aria-live="polite"
          >
            Exercício {Math.min(focusedExerciseIndex + 1, activeDraft.length)}/{activeDraft.length}:{' '}
            {activeDraft[focusedExerciseIndex]?.name ?? '-'}
          </p>
          <div className="flex items-center gap-2">
            {showMediaEnhancements && <RetroSoundToggle />}
            {showMediaEnhancements && pipGuard.canRenderControl && (
              <button
                type="button"
                onClick={() => {
                  void requestPictureInPictureForVideo();
                }}
        ...
        {showCameraFeedback && (
          ...
          >
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              ...
              <span
                className={`rounded-full border px-3 py-2 font-mono text-[10px] uppercase tracking-widest ${
        ...
        <section className="space-y-6">
          {activeDraft.map((exercise, eIdx) => (
            <article
              ...
              ref={(node) => {
                exerciseRefs.current[eIdx] = node;
              }}
              aria-current={focusedExerciseIndex === eIdx ? 'true' : undefined}
              className={`rounded-[28px] border-2 transition-all duration-300 ${
                exercise.completed
                  ? 'glass-panel-neon border-brand-neon/50 scale-[1.01] animate-card-complete'
                  : 'bg-brand-gray border-brand-light/15 shadow-brutal-light'
            ...
            >
              <div className="mb-4 flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                <div className="flex-1">
                  ...
                  {showMediaEnhancements && focusedExerciseIndex === eIdx && (
                  ...
                  <div className="mt-2 flex items-center gap-3">
                    ...
                    {exercise.plateauDetected && (
                      <span
                        className="flex items-center gap-1 rounded-full bg-brand-magenta/20 border border-brand-magenta text-brand-light px-3 py-1 font-mono text-[10px] uppercase tracking-widest"
                        title={exercise.plateauReason}
                      >
                ...
                <label className="flex items-center gap-3 cursor-pointer bg-brand-dark/50 px-4 py-2 rounded-full border border-brand-light/10">
                  <span className="font-mono text-xs uppercase tracking-widest text-brand-light">
                    Concluir Exercício
                  </span>
                  <input
                ...
                <table className="w-full text-left font-mono text-xs md:text-sm">
                  ...
                  <tbody>
                    {exercise.sets.map((set, sIdx) => (
                      <tr
                        key={sIdx}
                        className={`border-b border-brand-light/5 transition-colors ${set.completed ? 'bg-brand-neon/5 animate-touch-pop' : 'hover:bg-brand-dark/50'}`}
                      >
                        ...
                        <td className="py-3 px-2">
                          <input
                            ...
                            value={set.weight}
                            onChange={(event) =>
                              onUpdateDraftSet(eIdx, sIdx, { weight: event.target.value })
                            }
                            inputMode="decimal"
                          ...
                          {set.autofillSuggested && (
                            <div className="mt-1 flex flex-col gap-1">
                              <span className="text-[10px] text-brand-muted">
                                Sugestão: {set.suggestedWeight || '-'} kg /{' '}
                                {set.suggestedReps || '-'} reps / RPE {set.suggestedRpe || '-'}
                              </span>
                              <button
                                type="button"
                                onClick={() =>
                                  onUpdateDraftSet(eIdx, sIdx, applyAutofillSuggestion(set))
                                }
                              ...
                              <button
                                type="button"
                                onClick={() =>
                                  onUpdateDraftSet(eIdx, sIdx, { autofillSuggested: false })
                                }
                        ...
                        <td className="py-3 px-2">
                          <input
                            ...
                            value={set.reps}
                            onChange={(event) =>
                              onUpdateDraftSet(eIdx, sIdx, { reps: event.target.value })
                            }
                            inputMode="numeric"
                            className="w-16 md:w-20 rounded-[12px] border-2 border-brand-light/15 bg-brand-dark px-3 py-2 text-brand-light outline-none transition-colors placeholder:text-brand-muted/50 focus:border-brand-neon"
                            placeholder="0"
                        ...
                        <td className="py-3 px-2 relative">
                          <div className="flex items-center gap-1">
                            <input
                              ...
                              value={set.rpe}
                              onChange={(event) =>
                                onUpdateDraftSet(eIdx, sIdx, { rpe: event.target.value })
                              }
                              inputMode="decimal"
                            ...
                            {showRpeCalc?.eIdx === eIdx && showRpeCalc?.sIdx === sIdx && (
                              <div className="absolute top-12 left-0 z-50 w-56 glass-panel p-4 rounded-[20px] shadow-2xl border border-brand-light/20">
                                <div className="flex justify-between items-center mb-3">
                                  <p className="font-mono text-[10px] uppercase tracking-widest text-brand-neon">
                                    Reps na Reserva (RIR)
                                  </p>
                                  <button
                                    onClick={() => setShowRpeCalc(null)}
                          ...
                          <p className="mt-1 text-[10px] text-brand-muted">
                            {getRpeGuidance(set.rpe).guidance}
                          </p>
                        </td>
                        <td className="py-3 px-2 text-brand-light/80">
                          {calculateSetVolume(set) > 0
                            ? `${Math.round(calculateSetVolume(set)).toLocaleString('pt-BR')} kg`
                            : '-'}
                        </td>
                        <td className="py-3 px-2 text-center">
                          <label className="relative inline-flex cursor-pointer items-center justify-center">
                            <input
                              ...
                              onChange={(e) =>
                                handleSetCompletion(eIdx, sIdx, exercise, e.target.checked)
                              }
              ...
              <div
                className={`mt-5 grid gap-3 ${showMediaEnhancements ? 'md:grid-cols-[1fr_0.8fr]' : ''}`}
                data-swipe-ignore="true"
              >
                <label>
        ...
        <section className="mt-6 rounded-[28px] border-4 border-brand-magenta glass-panel-magenta p-6 shadow-brutal-magenta">
          <div className="mb-4 flex flex-wrap items-center gap-3">
            <button
              ...
              className="rounded-full border border-brand-light/20 px-4 py-2 font-mono text-[11px] uppercase"
            >
              Iniciar descanso
            </button>
            <button
      ...
      {(isRunning || isExpired) && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 glass-panel-neon px-6 py-3 rounded-full flex items-center gap-4 animate-bounce-subtle">
          <Clock
            className={`w-6 h-6 ${remainingSeconds === 0 ? 'text-brand-magenta animate-pulse' : 'text-brand-neon'}`}
          />
          <span className="font-mono text-2xl font-black tracking-widest text-brand-light">
            {formatted}
          </span>
          {remainingSeconds === 0 && (
            <span className="font-mono text-xs uppercase tracking-widest text-brand-magenta ml-2">
              Próxima série!
            </span>
          )}
          <button
