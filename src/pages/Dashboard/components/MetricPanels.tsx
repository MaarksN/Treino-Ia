...
export function MetricCard({
  ...
  return (
    <div className={`border-2 bg-brand-dark p-5 ${toneClass}`}>
      ...
      <p className="mt-2 font-mono text-[10px] uppercase tracking-widest text-brand-muted">
        {label}
      </p>
    </div>
  );
...
export function MetricPanel({
  ...
  return (
    <article className={`rounded-[28px] border-2 bg-brand-gray/80 p-6 ${toneClass}`}>
      ...
      <h3 className="mt-3 font-display text-4xl uppercase leading-none text-brand-light">
        {value}
      </h3>
    </article>
  );
