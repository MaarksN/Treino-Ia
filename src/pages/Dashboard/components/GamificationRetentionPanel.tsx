import { Award, BadgeCheck, Eye, Flame, LockKeyhole, ShieldCheck, Trophy } from 'lucide-react';
          ...
          <article className="border-2 border-brand-light/20 bg-brand-dark p-5">
            ...
            <div className="space-y-3">
              {state.leaderboard.map((entry) => (
                <div
                  key={entry.id}
                  className="grid gap-3 border border-brand-light/10 bg-brand-gray p-3 sm:grid-cols-[48px_1fr_auto] sm:items-center"
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 font-mono text-xs text-brand-muted">
                      {entry.rangeLabel} · {entry.workouts}/{entry.target} dias ·{' '}
                      {Math.round(entry.completionRate * 100)}% conclusao
                    </p>
              ...
              {state.badges.map((badge) => (
                <div key={badge.id} className="border border-brand-light/10 bg-brand-gray p-3">
                  ...
                  <ProgressBar
                    ...
                    tone={badge.achieved ? 'neon' : 'magenta'}
                    compact
                  />
                </div>
              ))}
          ...
          <article className="border-2 border-brand-light/20 bg-brand-dark p-5">
            <div className="mb-4 flex items-center gap-3">
              ...
              <div>
                ...
                <p className="font-mono text-xs text-brand-muted">
                  Geradas pela data e pelo historico local.
                </p>
              </div>
            </div>
              ...
              {state.hiddenMissions.map((mission) => (
                <div key={mission.id} className="border border-brand-light/10 bg-brand-gray p-3">
                  ...
                  <ProgressBar
                    ...
                    tone={mission.completed ? 'neon' : 'magenta'}
                    compact
                  />
                </div>
              ))}
...
function ProgressBar({
  ...
  return (
    <div
      className={`${compact ? 'mt-2 h-2' : 'mt-4 h-3'} w-full border border-brand-light/10 bg-brand-dark`}
    >
      <div
