...
export function NutritionLifestyleHub({
  ...
  const recipes = useMemo(
    () =>
      selectRecipesForMacroTargets(macros, {
        maxRecipes: 3,
        preferTags:
          recommendation.level === 'alto' ? ['fadiga', 'pre-treino'] : ['performance', 'base'],
      }),
  ...
  const handlePhotoAnalysis = (file: File) => {
    ...
    reader.onload = async () => {
      ...
      try {
        ...
        setScanText(
          'Não consegui analisar a foto agora. Verifique a chave Gemini e tente novamente.',
        );
      } finally {
    ...
    <section className="mb-8 rounded-[28px] border-4 border-brand-neon bg-brand-gray p-6 shadow-brutal-neon md:p-8">
      <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h2 className="mt-2 font-display text-5xl uppercase text-brand-light">
            Macros e hidratação
          </h2>
          <p className="mt-3 max-w-3xl font-mono text-sm leading-7 text-brand-light/75">
            {plan.planName} usa {profile.daysPerWeek} dias por semana; último treino:{' '}
            {latestSession ? latestSession.dayName : 'sem sessão finalizada'}.
          </p>
        </div>
        <label className="w-full rounded-[20px] border-2 border-brand-light/10 bg-brand-dark p-3 md:w-40">
          ...
          <input
            ...
            value={bodyWeightKg}
            onChange={(event) =>
              setBodyWeightKg(Math.min(180, Math.max(35, Number(event.target.value) || 75)))
            }
      ...
      <div className="grid gap-5 lg:grid-cols-[1fr_0.85fr]">
          ...
          <div className="rounded-[24px] border-2 border-brand-light/10 bg-brand-dark p-5">
            ...
            <div className="mt-4 grid gap-3 md:grid-cols-2">
              <div className="border border-brand-light/10 bg-white/5 p-4">
                ...
                <p className="mt-2 text-sm leading-6 text-brand-light/85">
                  {recommendation.preWorkout}
                </p>
              </div>
              <div className="border border-brand-light/10 bg-white/5 p-4">
                <p className="font-mono text-xs uppercase tracking-widest text-brand-muted">
                ...
                <p className="mt-2 text-sm leading-6 text-brand-light/85">
                  {recommendation.trainingAdjustment}
                </p>
              </div>
            </div>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {recommendation.mealIdeas.map((idea) => (
                <span
                  key={idea}
                  className="border border-brand-light/10 bg-brand-gray px-3 py-2 font-mono text-xs text-brand-light/80"
                >
                  {idea}
                </span>
              ))}
            ...
            {phaseConfig && todayPhase && (
              <div className="mt-4 border border-brand-light/10 bg-white/5 p-4">
                ...
                <p className="mt-1 font-bold text-brand-light" style={{ color: phaseConfig.color }}>
                  {phaseConfig.label} · dia {todayPhase.dayOfCycle} · energia{' '}
                  {todayPhase.energyExpected}
                </p>
              </div>
            )}
          </div>
              ...
              {recipes.recipes.map((recipe) => (
                <div key={recipe.id} className="border border-brand-light/10 bg-brand-gray p-4">
                  ...
                  <p className="mt-2 font-mono text-xs text-brand-muted">
                    {recipe.calories} kcal · P {recipe.proteinG}g · C {recipe.carbsG}g · G{' '}
                    {recipe.fatG}g
                  </p>
                </div>
              ))}
            ...
            <div className="mt-4 grid gap-2 md:grid-cols-2">
              {recipes.shoppingList.slice(0, 8).map((item) => (
                <div
                  ...
                  className="flex items-center justify-between border border-brand-light/10 bg-white/5 px-3 py-2"
                >
              ...
              {scanInsight && (
                <div className="mb-3 border border-brand-neon/30 bg-brand-neon/10 p-4">
                  <p className="font-mono text-xs uppercase tracking-widest text-brand-neon">
                    Veredito {scanInsight.verdict}
                  </p>
                  <p className="mt-2 text-sm text-brand-light/85">
                    {scanInsight.message} {scanInsight.nextAction}
                  </p>
                </div>
              )}
              {scanText && (
                <p className="whitespace-pre-wrap font-mono text-sm leading-6 text-brand-light/75">
                  {scanText}
                </p>
              )}
            </div>
        ...
        <aside className="space-y-5">
          <div className="rounded-[24px] border-2 border-brand-light/10 bg-brand-dark p-5">
            ...
            {hydrationNotice && (
              <p className="mt-3 font-mono text-xs text-brand-neon">{hydrationNotice}</p>
            )}
          </div>
      ...
      {showAdvanced && (
        <>
          <div className="grid gap-6 md:grid-cols-2 mt-6">
            <MicrobiotaWidget
              dailyFiberGrams={(macros.calories / 1000) * 14}
