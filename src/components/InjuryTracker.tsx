...
export function InjuryTracker() {
  const [injuries, setInjuries] = useState<InjuryRecord[]>(() =>
    loadJSON<InjuryRecord[]>(INJURY_KEY, []),
  );
  const [symptoms, setSymptoms] = useState<SymptomRecord[]>(() =>
    loadJSON<SymptomRecord[]>(SYMPTOM_KEY, []),
  );
        ...
        <>
          <div className="space-y-3 mb-5 p-4 bg-brand-dark border-2 border-brand-light/10">
            ...
            <select
              ...
              onChange={(event) =>
                setNewInjury((current) => ({ ...current, region: event.target.value }))
              }
            ...
            <input
              ...
              onChange={(event) =>
                setNewInjury((current) => ({ ...current, description: event.target.value }))
              }
              className="w-full bg-brand-gray border-2 border-brand-light/10 px-3 py-2 text-sm text-brand-light outline-none focus:border-brand-neon"
            />
            <select
              ...
              onChange={(event) =>
                setNewInjury((current) => ({
                  ...current,
                  severity: event.target.value as InjuryRecord['severity'],
                }))
              }
              className="w-full bg-brand-gray border-2 border-brand-light/10 px-3 py-2 text-sm text-brand-light outline-none focus:border-brand-neon"
            >
            ...
            <button
              onClick={addInjury}
              type="button"
              className="inline-flex items-center gap-2 bg-brand-neon text-brand-dark px-4 py-2 border-brutal text-sm font-bold uppercase"
            >
          ...
          <div className="space-y-2">
            {unresolvedInjuries.map((injury) => (
              <div
                key={injury.id}
                className="flex items-start justify-between gap-3 p-3 bg-brand-dark border-2 border-brand-light/10"
              >
                <div>
                  ...
                  <span className={`text-xs font-bold ${severityColor[injury.severity]}`}>
                    {injury.severity}
                  </span>
                ...
                <button
                  ...
                  className="p-2 bg-green-500/10 text-green-400 hover:bg-green-500/20 transition-colors"
                >
                  <Check size={14} />
                </button>
              </div>
            ))}
            {unresolvedInjuries.length === 0 && (
              <p className="text-brand-muted text-sm">Nenhuma lesão ativa registrada.</p>
            )}
          </div>
        </>
      )}
        ...
        <>
          <div className="space-y-3 mb-5 p-4 bg-brand-dark border-2 border-brand-light/10">
            ...
            <select
              ...
              onChange={(event) =>
                setNewSymptom((current) => ({ ...current, region: event.target.value }))
              }
              className="w-full bg-brand-gray border-2 border-brand-light/10 px-3 py-2 text-sm text-brand-light outline-none focus:border-brand-neon"
            >
              <option value="">Região</option>
            ...
            <input
              ...
              onChange={(event) =>
                setNewSymptom((current) => ({ ...current, symptom: event.target.value }))
              }
              className="w-full bg-brand-gray border-2 border-brand-light/10 px-3 py-2 text-sm text-brand-light outline-none focus:border-brand-neon"
            />
          ...
          <div className="space-y-2 max-h-64 overflow-y-auto">
              ...
              <div key={symptom.id} className="p-3 bg-brand-dark border-2 border-brand-light/10">
                <div className="flex justify-between gap-3">
                  <p className="text-brand-light text-sm font-semibold">
                    {symptom.region} - {symptom.symptom}
                  </p>
                  <span
                    className={`text-xs font-bold ${symptom.intensity >= 7 ? 'text-red-400' : symptom.intensity >= 4 ? 'text-orange-400' : 'text-green-400'}`}
                  >
            ...
            {symptoms.length === 0 && (
              <p className="text-brand-muted text-sm">Sem sintomas registrados.</p>
            )}
          </div>
        </>
      )}
