    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 mt-8">
      <div className="bg-brand-gray border-4 border-brand-light shadow-[8px_8px_0px_var(--color-brand-light)] p-8 md:p-12 text-center mb-12 relative overflow-hidden group hover:border-brand-neon transition-colors">
        ...
        <h1 className="font-display font-black text-7xl md:text-9xl tracking-tighter uppercase mb-4 text-brand-light relative z-10 transition-transform text-shadow-neon">
          E aí,{' '}
          <span className="text-brand-neon bg-brand-dark px-4 py-2 border-2 border-brand-neon inline-block mt-2 md:mt-0 shadow-brutal-neon">
            {user.name.split(' ')[0]}
          </span>
          !
        </h1>
        <p className="text-brand-magenta font-mono font-bold text-xl md:text-2xl relative z-10 uppercase tracking-widest mt-6">
          O que vamos destruir hoje?
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-12">
        <div className="bg-brand-dark border-2 border-brand-neon p-6 text-center hover:bg-brand-neon/5 transition-colors cursor-default">
          ...
          <p className="text-[10px] text-brand-muted uppercase font-mono tracking-widest mt-1">
            Treinos
          </p>
        </div>
        <div className="bg-brand-dark border-2 border-brand-magenta p-6 text-center hover:bg-brand-magenta/5 transition-colors cursor-default">
          ...
          <h4 className="text-brand-light font-display text-4xl font-black">
            14<span className="text-xl">h</span>
          </h4>
          <p className="text-[10px] text-brand-muted uppercase font-mono tracking-widest mt-1">
            Tempo na Forja
          </p>
        ...
        <div className="bg-brand-dark border-2 border-brand-light p-6 text-center hover:bg-brand-light/5 transition-colors cursor-default">
          ...
          <h4 className="text-brand-light font-display text-4xl font-black">
            2.5<span className="text-xl">k</span>
          </h4>
          <p className="text-[10px] text-brand-muted uppercase font-mono tracking-widest mt-1">
            Kcal Queimadas
          </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
        ...
        >
          ...
          <h2 className="font-display font-black text-5xl uppercase tracking-tighter mb-3 text-brand-light group-hover:text-brand-dark transition-colors text-shadow-neon group-hover:text-shadow-none">
            Criar com IA
          </h2>
          <p className="text-brand-muted font-mono text-sm px-4 text-center group-hover:text-brand-dark/80 transition-colors">
            Faça uma avaliação. Deixe o algoritmo esculpir uma rotina insana perfeitamente para
            você.
          </p>
        </button>
        ...
        >
          ...
          <p className="text-brand-muted font-mono text-sm px-4 text-center group-hover:text-brand-dark/80 transition-colors">
            Envie uma foto da sua ficha atual. O sistema traduz e otimiza instantaneamente.
          </p>
        </button>
      </div>
      ...
      <div className="flex flex-col md:flex-row items-center justify-between p-8 bg-brand-dark border-4 border-brand-light shadow-[6px_6px_0px_var(--color-brand-light)]">
        <div className="text-center md:text-left mb-6 md:mb-0 max-w-sm">
          ...
          <p className="text-brand-muted font-mono text-sm">
            Receba notificações implacáveis para nunca pular os dias de perna.
          </p>
          {notiMessage && (
            <p className="text-xs text-brand-magenta font-mono font-bold mt-4 bg-brand-magenta/10 p-2 inline-block animate-pulse">
              {notiMessage}
            </p>
          )}
        </div>
      ...
      {showPaywall && (
        <PremiumPaywall
          trigger={{
            ...
            description:
              'Assinantes Free podem criar no máximo 2 planos de treino ativos. Faça upgrade para planos ilimitados.',
          }}
