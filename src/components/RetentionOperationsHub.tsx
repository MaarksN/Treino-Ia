...
import { ReminderChannel, ReminderType, RetentionHubState } from '../types/retention';
import {
...
export function RetentionOperationsHub({
  userName = 'Atleta',
  profile,
  currentPlan,
  history = [],
  ...
  const [checkinBody, setCheckinBody] = useState(
    'Como você dormiu, treinou e se recuperou desde o último check-in?',
  );
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(
        loadError instanceof Error ? loadError.message : 'Não foi possível carregar retenção.',
      );
    } finally {
      setLoading(false);
    }
    ...
    runAction(
      ...
      () =>
        saveReminder({
          ...
          schedule: {
            daysOfWeek:
              reminderType === 'hydration' || reminderType === 'reactivation'
                ? undefined
                : [0, 1, 2, 3, 4, 5, 6],
            time:
              reminderType === 'hydration' || reminderType === 'reactivation'
  ...
  const startChallenge = (days: 7 | 14 | 30) =>
    runAction(
      `challenge-${days}`,
  ...
  const createAlternative = (durationMinutes: number) =>
    runAction(
  ...
  const markIntegrationPending = (integration: RetentionHubState['integrations'][number]) =>
    runAction(
      `integration-${integration.provider}`,
      async () => {
        if (
          integration.data_mode === 'oauth' &&
          ['google_fit', 'fitbit', 'strava'].includes(integration.provider)
        ) {
          const result = await startHealthOAuth(
            integration.provider as 'google_fit' | 'fitbit' | 'strava',
          );
  ...
  return (
    <div className="min-h-screen bg-brand-dark text-white p-4 md:p-6">
      <header className="max-w-7xl mx-auto mb-8">
        <p className="text-brand-neon text-xs uppercase tracking-[0.25em] font-bold">Bloco 05</p>
        <h1 className="text-4xl font-black mt-2">Retenção, Hábito e Operação</h1>
        <p className="text-brand-muted mt-2 max-w-3xl">
          Streaks, metas, lembretes, agenda, social e operação de alunos com persistência
          Supabase/RLS.
        </p>
          ...
          <>
            <section className="grid md:grid-cols-4 gap-4">
              ...
              <Metric
                ...
                value={`${metrics.weeklyStreak}sem`}
              />
              <Metric
                icon={<Activity />}
                label="Treinos na semana"
                value={`${metrics.workoutsThisWeek}/${state.profile.consistency_workouts_per_week}`}
              />
                ...
                <div className="grid md:grid-cols-2 gap-3">
                  ...
                  <label className="text-sm text-brand-muted">
                    ...
                    <select
                      ...
                      onChange={(event) =>
                        setReminderChannel(event.target.value as ReminderChannel)
                      }
                      className="mt-2 w-full bg-brand-dark border border-white/10 rounded-xl px-3 py-3 text-white outline-none"
                    >
                  ...
                  {reminderType === 'hydration' ? (
                    <NumberField
                      ...
                      max={240}
                      step={15}
                      onChange={setReminderEvery}
                    />
                  ) : reminderType === 'reactivation' ? (
                    <NumberField
                      label="Dias inativo"
                      value={reminderInactivity}
                      min={2}
                      max={30}
                      onChange={setReminderInactivity}
                    />
                  ) : (
                    <label className="text-sm text-brand-muted">
                      Horário
              ...
              <div className="bg-brand-gray border border-white/10 rounded-2xl p-5">
                ...
                <div className="mt-4 grid gap-2">
                  ...
                  <ActionButton
                    ...
                    saving={saving === 'hydration'}
                    icon={<Droplets size={16} />}
                    label="Registrar hidratação"
                  />
                  <ActionButton
                    onClick={logSleep}
                    saving={saving === 'sleep'}
                ...
                <p className="mt-3 text-xs text-brand-muted">
                  Hoje: {metrics.hydrationTodayMl}ml · sono médio{' '}
                  {formatMinutes(metrics.sleepAverageMinutes)}
                </p>
              </div>
              ...
              <div className="bg-brand-gray border border-white/10 rounded-2xl p-5">
                ...
                <div className="space-y-2 max-h-56 overflow-y-auto">
                  {state.challenges.map((challenge) => (
                    <div
                      key={challenge.id}
                      className="bg-brand-dark border border-white/10 rounded-xl p-3"
                    >
                      <div className="flex justify-between gap-3 text-sm">
                        <span className="text-white font-bold">{challenge.title}</span>
                        <span className="text-brand-neon">
                          {challenge.progress ?? 0}/{challenge.target_days}
                        </span>
                      </div>
                      <p className="text-xs text-brand-muted">
                        {formatDate(challenge.starts_on)} até {formatDate(challenge.ends_on)} ·{' '}
                        {challenge.status}
                      </p>
                    </div>
                  ))}
                  {!state.challenges.length && (
                    <p className="text-sm text-brand-muted">Nenhum desafio iniciado.</p>
                  )}
                </div>
              </div>
            ...
            <section className="grid xl:grid-cols-2 gap-5">
              <div className="bg-brand-gray border border-white/10 rounded-2xl p-5">
                ...
                <div className="mt-4 space-y-2 max-h-52 overflow-y-auto">
                  {state.calendar.map((item) => (
                    <div
                      key={item.id}
                      className="bg-brand-dark border border-white/10 rounded-xl p-3 flex justify-between gap-3"
                    >
                      <div>
                        <p className="text-white font-bold">{item.title}</p>
                        <p className="text-xs text-brand-muted">
                          {formatDate(item.scheduled_for)} · {item.time_of_day ?? '--:--'} ·{' '}
                          {item.source}
                        </p>
                      </div>
                  ...
                  {!state.calendar.length && (
                    <p className="text-sm text-brand-muted">Agenda vazia.</p>
                  )}
                </div>
              </div>
              ...
              <div className="bg-brand-gray border border-white/10 rounded-2xl p-5">
                ...
                <p className="mt-3 text-xs text-brand-muted">
                  Pendentes:{' '}
                  {state.automatedCheckins.filter((item) => item.status === 'pending').length}
                </p>
              </div>
            </section>
                ...
                <div className="space-y-2">
                  {state.integrations.map((integration) => (
                    <div
                      ...
                      className="bg-brand-dark border border-white/10 rounded-xl p-3 flex items-center justify-between gap-3"
                    >
                      <div>
                        <p className="text-white font-bold">{integration.provider}</p>
                        <p className="text-xs text-brand-muted">
                          {integration.status} · {integration.data_mode}
                        </p>
                      </div>
                      <button
                        type="button"
              ...
              <div className="grid md:grid-cols-[1fr_auto] gap-4 items-center">
                <div>
                  <p className="text-brand-muted">
                    Etapa {state.onboarding?.current_step ?? 0}/{state.onboarding?.total_steps ?? 7}{' '}
                    · {state.onboarding?.completed ? 'concluído' : 'em andamento'}
                  </p>
                  <p className="text-xs text-white/40 mt-1">
                    Atualizado em{' '}
                    {state.onboarding?.updated_at
                      ? new Date(state.onboarding.updated_at).toLocaleString('pt-BR')
                      : '-'}
                  </p>
                </div>
                <button
...
function Metric({
  ...
  return (
    <div className={`bg-brand-gray border border-white/10 rounded-2xl ${compact ? 'p-3' : 'p-5'}`}>
      ...
      <p className={`${compact ? 'text-xl' : 'text-3xl'} font-black text-white mt-1 tabular-nums`}>
        {value}
      </p>
    </div>
  );
