    ...
    return (
      <div className="bg-brand-gray border-2 border-brand-light/10 p-5 shadow-brutal-light">
        ...
        <p className="text-brand-muted text-sm">
          Faça o check-in diário para calcular seu índice de prontidão.
        </p>
      </div>
    );
  }
    ...
    <div className="bg-brand-gray border-2 border-brand-light/10 p-5 shadow-brutal-light">
      ...
      <div className="flex items-center gap-6 mb-5">
        ...
        <div className="min-w-0">
          <p className="text-3xl font-black" style={{ color: readiness.color }}>
            {readiness.label}
          </p>
          <p className="text-brand-muted text-sm mt-1">{readiness.recommendation}</p>
          <p className="text-brand-muted text-xs mt-1">
            Intensidade: {readiness.adjustedIntensity}%
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
        {[
          ...
          {
            label: 'Energia',
            value: `${checkin.energyLevel}/10`,
            Icon: Zap,
            ok: checkin.energyLevel >= 6,
          },
          {
            label: 'Estresse',
            value: `${checkin.stressLevel}/10`,
            Icon: Battery,
            ok: checkin.stressLevel <= 6,
          },
          {
            label: 'Água',
            value: `${checkin.hydrationGlasses}`,
            Icon: Droplets,
            ok: hydrationGoalHit,
          },
          ...
          <div key={label} className="bg-brand-dark border-2 border-brand-light/10 p-3 text-center">
            <Icon
              className={`w-5 h-5 mx-auto mb-2 ${ok ? 'text-brand-neon' : 'text-orange-400'}`}
            />
      ...
      <div className={`flex items-center gap-2 p-3 border-2 text-sm ${riskClass}`}>
        ...
        <span>
          Risco de overtraining: <strong>{risk}</strong>
        </span>
      </div>
    </div>
  );
