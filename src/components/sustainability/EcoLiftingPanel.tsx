    ...
    <div className="rounded-[24px] border-2 border-brand-neon bg-brand-dark p-6 shadow-brutal-neon">
      <div className="flex items-center gap-4 border-b-2 border-brand-neon/20 pb-4">
        ...
        <div>
          ...
          <p className="font-mono text-xs text-brand-light/70">
            Gamificação sustentável baseada no seu compromisso.
          </p>
        </div>
      </div>
      ...
      <div className="mt-6 flex flex-col gap-6 md:flex-row">
        <div className="flex-1 space-y-4">
          ...
          <p className="font-mono text-sm leading-relaxed text-brand-light/80">{stats.message}</p>
        </div>

        <div className="flex-1">
          <h4 className="mb-3 font-mono text-xs uppercase tracking-widest text-brand-light">
            Suas Conquistas
          </h4>
          {stats.badges.length > 0 ? (
            <ul className="space-y-2">
              {stats.badges.map((badge) => (
                <li
                  key={badge}
                  className="flex items-center gap-2 rounded-full border border-brand-neon/30 bg-brand-neon/5 px-3 py-2 font-mono text-xs text-brand-neon"
                >
