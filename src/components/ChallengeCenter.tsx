      ...
      <div className="space-y-3">
        ...
        {activeMissions.map((mission) => {
          ...
          return (
            <div
              key={mission.id}
              className={`p-4 border-2 transition-colors ${isCompleted ? 'border-brand-neon/50 bg-brand-neon/5' : 'border-brand-light/10 bg-brand-dark'}`}
            >
              <div className="flex items-start justify-between mb-2 gap-3">
                ...
                <div className="text-right shrink-0">
                  <p className="text-brand-neon font-bold text-sm tabular-nums">
              ...
              <div className="h-2 bg-white/10 overflow-hidden mb-2">
                <div
                  ...
                  style={{ width: `${pct}%`, background: isCompleted ? '#a3e635' : '#60a5fa' }}
                />
              </div>

              <div className="flex items-center justify-between mt-2">
                <div className="flex gap-2">
                  <span className="px-2 py-0.5 bg-brand-neon/10 text-brand-neon border border-brand-neon/20 text-[10px] font-bold uppercase rounded-sm">
                    +{mission.xpReward} XP
                  </span>
                  <span className="px-2 py-0.5 bg-yellow-400/10 text-yellow-400 border border-yellow-400/20 text-[10px] font-bold uppercase rounded-sm">
              ...
              {mission.expiresAt && (
                <p className="text-brand-light/30 text-[10px] mt-2">
