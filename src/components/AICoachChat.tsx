...
export function AICoachChat({ user, onClose }: Props) {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'model',
      text: `Fala, ${user.name.split(' ')[0]}! Aqui é a Inteligência Artificial Supremo. No meio do treino e bateu dúvida? Manda aqui. Como posso esmagar sua incerteza hoje? 💀`,
    },
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const systemInstruction =
        'Você é o Coach Supremo IA. Você dá respostas curtas, diretas, ríspidas (mas engraçadas e motivacionais) sobre treino. Fale como um treinador old-school cibernético. Não de discursos longos. Aja rápido. Sempre termine com uma dica de ouro brutal e um emoji.';
      ...
      setMessages((prev) => [
        ...prev,
        { role: 'model', text: 'Tive uma falha no sistema neural. Tente de novo! 🤖' },
      ]);
    } finally {
      setIsLoading(false);
    }
    ...
    <AnimatePresence>
      ...
      >
        <div className="bg-brand-neon text-brand-dark p-3 flex justify-between items-center px-4 shrink-0">
          ...
          <button onClick={onClose} className="hover:text-brand-light transition-colors">
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <PremiumFeatureGate feature="unlimited_ai">
          <div className="flex-1 overflow-y-auto p-4 space-y-4" ref={scrollRef}>
            {messages.map((m, i) => (
              ...
              >
                <div
                  className={`max-w-[85%] p-3 text-sm font-mono flex items-start border-2 ${m.role === 'user' ? 'bg-brand-gray border-brand-light/20 text-brand-light' : 'bg-brand-neon/10 border-brand-neon text-brand-light'}`}
                >
                  {m.role === 'model' && (
                    <Bot className="w-4 h-4 mr-2 shrink-0 border border-brand-neon rounded bg-brand-dark p-0.5" />
                  )}
                  <p className="leading-relaxed">{m.text}</p>
                  {m.role === 'user' && (
                    <UserIcon className="w-4 h-4 ml-2 shrink-0 text-brand-muted" />
                  )}
                </div>
              </div>
            ))}
