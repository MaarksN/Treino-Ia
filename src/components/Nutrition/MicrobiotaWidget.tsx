...
export const MicrobiotaWidget: React.FC<MicrobiotaWidgetProps> = ({
  dailyFiberGrams,
  calories,
}) => {
  ...
  return (
    ...
    >
        ...
        <div className="mb-4 flex flex-col sm:flex-row sm:items-center gap-4 border border-brand-light/10 bg-brand-gray p-4 rounded-[16px]">
          ...
          <div>
            ...
            <p className="font-display text-2xl text-brand-neon">
              {Math.round(insight.recommendedFibersGrams)}g{' '}
              <span className="font-mono text-[10px] text-brand-muted">/dia</span>
            </p>
          </div>
        </div>
      )}
      ...
      <InlineNotice type="info" title="Educacional">
        Estas estimativas são baseadas na meta de 14g a cada 1000kcal e não substituem diagnóstico
        médico nutricional.
      </InlineNotice>
    </div>
  );
