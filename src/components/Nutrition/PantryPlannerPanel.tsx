...
import {
  ...
  savePantryItems,
  suggestMeals,
} from '../../services/nutrition/pantryPlannerService';
...
export function PantryPlannerPanel() {
  ...
  return (
    ...
    >
      <h3 id="pantry-title" className="font-display text-3xl uppercase text-brand-light">
        Despensa inteligente
      </h3>
      <p className="mt-1 font-mono text-xs text-brand-muted">
        Marque o que tem em casa e receba sugestões de refeição.
      </p>
      <div className="mt-4 flex flex-wrap gap-2">
        {PANTRY_ITEMS.map((id) => (
          <button
            ...
            type="button"
            onClick={() => toggle(id)}
            aria-pressed={pantry.includes(id)}
      ...
      {pantry.length > 0 && (
        <p className="mt-2 font-mono text-xs text-brand-neon" role="status">
          {pantry.length} ite{pantry.length !== 1 ? 'ns' : 'm'} na despensa.
        </p>
      )}
      {meals.length > 0 ? (
        <div className="mt-4 space-y-2">
          <p className="font-mono text-xs font-bold uppercase text-brand-neon">
            Refeições possíveis
          </p>
          {meals.map((meal) => (
            <div
        ...
        <p className="mt-3 font-mono text-xs text-brand-muted">
          Nenhuma receita encontrada com os itens selecionados. Adicione mais alimentos.
        </p>
      ) : null}
      <InlineNotice type="info" title="Sem IoT">
        {PANTRY_DISCLAIMER}
      </InlineNotice>
    </article>
  );
