...
import {
  calculateLongevitySignal,
  LONGEVITY_DISCLAIMER,
} from '../../services/wellness/longevitySignalService';
...
const LEVEL_COLORS: Record<string, string> = {
  excelente: 'text-brand-neon',
  bom: 'text-brand-neon',
  moderado: 'text-brand-gold',
  baixo: 'text-brand-magenta',
};
...
export function LongevitySignalPanel({
  ...
  return (
    ...
    >
      <h3 id="longevity-title" className="font-display text-3xl uppercase text-brand-light">
        Sinal de consistência
      </h3>
      <p className="mt-1 font-mono text-xs text-brand-muted">
        Indicador educativo de hábitos de treino.
      </p>
      <div className="mt-4 flex items-center gap-4">
        <div className="flex h-16 w-16 items-center justify-center rounded-full border-4 border-brand-neon/30 bg-brand-dark">
          <span
            className={`font-display text-2xl ${LEVEL_COLORS[signal.level] ?? 'text-brand-light'}`}
          >
        ...
        <div>
          <p
            className={`font-mono text-sm font-bold ${LEVEL_COLORS[signal.level] ?? 'text-brand-light'}`}
          >
            {signal.label}
          </p>
          <p className="font-mono text-[10px] text-brand-muted">
            Baseado em {history.length} sessões registradas.
          </p>
        </div>
      </div>
      ...
      <InlineNotice type="warning" title="Indicador educativo">
        {LONGEVITY_DISCLAIMER}
      </InlineNotice>
    </article>
  );
