...
export function PlanAutoAdjust({ plan, history, checkins, profile, onApplyAdjustment }: Props) {
  ...
  const analyze = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : 'Não foi possível analisar o plano agora.';
      setError(message);
    } finally {
      setLoading(false);
    }
        ...
        <div
          className={`p-3 rounded-xl border mb-4 ${
            priority.toLowerCase() === 'alta'
              ...
              : priority.toLowerCase() === 'média' || priority.toLowerCase() === 'media'
      ...
      <div className="space-y-3">
        {suggestions.map((suggestion, index) => {
          ...
          return (
            ...
            >
              ...
              {suggestion.affectedDay && (
                <p className="text-blue-400 text-xs">Dia: {suggestion.affectedDay}</p>
              )}
              {suggestion.affectedExercise && (
                <p className="text-orange-400 text-xs">Exercício: {suggestion.affectedExercise}</p>
              )}
        ...
        {suggestions.length === 0 && !loading && assessment === '' && !error && (
          <p className="text-brand-muted text-sm text-center py-4">
            Clique em analisar para receber sugestões.
          </p>
        )}
      </div>
    </div>
