...
} from '../../services/ai/equipmentReplanService';
...
export function EquipmentReplanPanel() {
  ...
  return (
    <article
      ...
      aria-labelledby="equip-replan-title"
    >
      <h3 id="equip-replan-title" className="font-display text-3xl uppercase text-brand-light">
        Equipamentos disponíveis
      </h3>
      <p className="mt-1 font-mono text-xs text-brand-muted">
        Selecione o que você tem e receba sugestões de adaptação.
      </p>
      <label className="mt-4 block rounded-2xl border-2 border-dashed border-brand-light/20 bg-brand-dark/30 p-4">
        <span className="block font-mono text-xs font-bold uppercase text-brand-light">
          Foto do espaço
        </span>
        <span className="mt-1 block font-mono text-[10px] text-brand-muted">
          Opcional. A imagem fica apenas como preview local; a adaptação usa a lista manual.
        </span>
        <input
          type="file"
          accept="image/*"
      ...
      {photoPreview && (
        <figure className="mt-3 overflow-hidden rounded-2xl border border-brand-light/10 bg-brand-dark/40">
          <img
            ...
            alt={`Preview da foto de equipamentos: ${photoPreview.name}`}
            className="h-40 w-full object-cover"
          />
          <figcaption className="px-3 py-2 font-mono text-[10px] text-brand-muted">
            {photoPreview.name}
          </figcaption>
        </figure>
      )}
      ...
      {adaptation.suggestions.length > 0 && (
        <div className="mt-4 space-y-1">
          ...
          {adaptation.suggestions.map((s, i) => (
            <p key={i} className="flex items-start gap-2 font-mono text-xs text-brand-muted">
      ...
      {selected.length > 0 && (
        <p className="mt-2 font-mono text-[10px] text-brand-muted">{adaptation.disclaimer}</p>
      )}
      <InlineNotice type="info" title="Foto apenas como referência">
        {EQUIPMENT_PHOTO_GUARD}
      </InlineNotice>
    </article>
  );
