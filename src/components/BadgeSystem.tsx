      ...
      {newlyUnlocked.length > 0 && (
        <div className="fixed top-4 right-4 z-50 space-y-2 max-w-sm">
          {newlyUnlocked.map((badge) => (
            <div
              key={badge.id}
              className="flex items-center gap-3 bg-brand-gray border-2 border-brand-neon/50 p-4 shadow-2xl"
            >
              <span className="text-4xl">{badge.emoji}</span>
              <div className="min-w-0">
                <p className="text-xs text-brand-neon font-bold uppercase tracking-widest">
              ...
              <button
                onClick={onDismiss}
                type="button"
                className="text-brand-muted hover:text-brand-light ml-2"
              >
                x
              </button>
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {filtered.map((badge) => {
            const badgeContent = (
              <div
                key={badge.id}
                className={`flex flex-col items-center p-3 border-2 text-center transition-all ${badge.unlocked ? 'border-brand-neon/30 bg-brand-neon/5' : 'border-brand-light/5 bg-brand-dark opacity-45 grayscale'}`}
              >
                ...
                {badge.unlocked && badge.unlockedAt && (
                  <p className="text-[10px] text-brand-neon mt-1">
            ...
            if (badge.category === 'special') {
              return (
                <PremiumFeatureGate
                  key={badge.id}
                  feature="exclusive_badge"
                  fallback={badgeContent}
                >
                  {badgeContent}
                </PremiumFeatureGate>
              );
