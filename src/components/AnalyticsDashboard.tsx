...
export function AnalyticsDashboard({ history, plans, workoutHistory = [], profile }: Props) {
  ...
  const muscleVolume = useMemo(
    () => getMuscleGroupVolume(history, plans).slice(0, 8),
    [history, plans],
  );
  ...
  const volumeData = weeklyStats.map((week) => ({
    name: week.weekLabel,
    ...
    <div className="bg-brand-gray border-2 border-brand-light/10 p-5 shadow-brutal-light">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 mb-4">
        ...
        <span className="text-[10px] text-brand-muted uppercase tracking-widest font-mono">
          Dados reais das sessões salvas
        </span>
      </div>
          ...
          <div className="grid md:grid-cols-2 gap-3">
            {indicators.map((item) => (
              <div key={item.label} className="p-4 bg-brand-dark border-2 border-brand-light/10">
                <p className={`text-2xl font-black tabular-nums ${toneClass(item.tone)}`}>
                  {item.value}
                </p>
          ...
          <div className="p-4 bg-brand-dark border-2 border-brand-light/10">
            ...
            <div className="h-3 bg-white/10 overflow-hidden">
              <div
                ...
                style={{
                  width: `${adherence}%`,
                  background: adherence >= 80 ? NEON : adherence >= 50 ? ORANGE : RED,
                }}
              />
            </div>
          </div>
            ...
            <div className="space-y-2">
              {[...history]
                ...
                .map((entry) => (
                  <div
                    key={entry.id}
                    className="flex items-center justify-between p-3 bg-brand-dark border-2 border-brand-light/10"
                  >
                    <div>
                    ...
                    <div className="text-right">
                      <p className="text-brand-neon text-sm font-bold">
        ...
        <div className="space-y-4">
          ...
          {volumeData.some((item) => item.volume > 0) ? (
            ...
            <p className="text-brand-muted text-sm text-center py-8">
              Registre treinos com carga para ver o gráfico.
            </p>
          )}
          ...
          <div className="h-40">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={weeklyStats.map((week) => ({ name: week.weekLabel, sessoes: week.sessions }))}
              >
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                <XAxis dataKey="name" stroke="#9ca9bb" tick={{ fontSize: 11 }} />
        ...
        <div className="space-y-5">
          <div>
            ...
            {topWeeklyGroups.length > 0 ? (
              ...
              <p className="text-brand-muted text-sm text-center py-6 bg-brand-dark border-2 border-brand-light/10">
                Registre treinos finalizados para ver volume semanal por músculo.
              </p>
            )}
          </div>
            ...
            {muscleVolume.length > 0 ? (
              <div className="space-y-2">
                {muscleVolume.map((item, index) => {
                  const pct = muscleVolume[0].volume
                    ? (item.volume / muscleVolume[0].volume) * 100
                    : 0;
                  return (
                    <div key={item.group}>
              ...
              <p className="text-brand-muted text-sm text-center py-8">
                Registre exercícios com grupo muscular para ver.
              </p>
            )}
          </div>
        </div>
      ...
      {tab === 'evolucao' && (
        <div className="space-y-4">
          ...
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
                data={weeklyRpe.map((week) => ({ name: week.weekLabel, rpe: week.avgRpe }))}
              >
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                <XAxis dataKey="name" stroke="#9ca9bb" tick={{ fontSize: 11 }} />
            ...
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
                data={weeklyStats.map((week) => ({
                  name: week.weekLabel,
                  readiness: Math.round(week.avgReadiness),
                }))}
              >
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                <XAxis dataKey="name" stroke="#9ca9bb" tick={{ fontSize: 11 }} />
          ...
          <div className="h-36">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={weeklyStats.map((week) => ({
                  name: week.weekLabel,
                  aderencia: Math.round(week.adherence * 100),
                }))}
              >
                <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                <XAxis dataKey="name" stroke="#9ca9bb" tick={{ fontSize: 11 }} />
                ...
                <Bar dataKey="aderencia">
                  {weeklyStats.map((week, index) => (
                    <Cell
                      key={index}
                      fill={week.adherence >= 0.8 ? NEON : week.adherence >= 0.5 ? ORANGE : RED}
                    />
                  ))}
                </Bar>
              </BarChart>
      ...
      {tab === 'radar' && (
        <div className="space-y-4">
          ...
          {radarData.length >= 3 ? (
            ...
            <p className="text-brand-muted text-sm text-center py-8">
              Registre treinos com grupo muscular para ver o radar.
            </p>
          )}
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="grid md:grid-cols-2 gap-3">
            <div className="bg-brand-dark border-2 border-brand-light/10 p-4">
              <p className="text-xs text-brand-muted uppercase tracking-widest mb-2">
                {comparison.previousWeekLabel}
              </p>
              <p className="text-brand-light text-sm">
                Sessões: <strong>{comparison.previous.sessions}</strong>
              </p>
              <p className="text-brand-light text-sm">
                Volume: <strong>{formatVolume(comparison.previous.totalVolume)}</strong>
              </p>
              <p className="text-brand-light text-sm">
                Aderência: <strong>{Math.round(comparison.previous.adherence * 100)}%</strong>
              </p>
              <p className="text-brand-light text-sm">
                RPE médio: <strong>{comparison.previous.avgRpe || '-'}</strong>
              </p>
            </div>
            <div className="bg-brand-dark border-2 border-brand-light/10 p-4">
              <p className="text-xs text-brand-muted uppercase tracking-widest mb-2">
                {comparison.currentWeekLabel}
              </p>
              <p className="text-brand-light text-sm">
                Sessões: <strong>{comparison.current.sessions}</strong>
              </p>
              <p className="text-brand-light text-sm">
                Volume: <strong>{formatVolume(comparison.current.totalVolume)}</strong>
              </p>
              <p className="text-brand-light text-sm">
                Aderência: <strong>{Math.round(comparison.current.adherence * 100)}%</strong>
              </p>
              <p className="text-brand-light text-sm">
                RPE médio: <strong>{comparison.current.avgRpe || '-'}</strong>
              </p>
            </div>
          </div>
            ...
            {[
              {
                ...
                value:
                  comparison.sessionsDelta >= 0
                    ? `+${comparison.sessionsDelta}`
                    : comparison.sessionsDelta,
                color: comparison.sessionsDelta >= 0 ? 'text-brand-neon' : 'text-orange-400',
              },
              {
                label: 'Volume',
                value: `${comparison.volumeDelta >= 0 ? '+' : ''}${formatVolume(comparison.volumeDelta)}`,
                color: comparison.volumeDelta >= 0 ? 'text-brand-neon' : 'text-orange-400',
              },
              {
                label: 'Aderência',
                value: `${comparison.adherenceDelta >= 0 ? '+' : ''}${comparison.adherenceDelta}%`,
                color: comparison.adherenceDelta >= 0 ? 'text-brand-neon' : 'text-orange-400',
              },
              {
                label: 'RPE',
                value: `${comparison.rpeDelta >= 0 ? '+' : ''}${comparison.rpeDelta}`,
                color: comparison.rpeDelta <= 0.5 ? 'text-blue-400' : 'text-orange-400',
              },
            ].map((item) => (
