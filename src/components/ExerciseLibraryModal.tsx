...
export function ExerciseLibraryModal({ onClose }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    if (muscleFilter !== 'Todos')
      list = list.filter((exercise) => exercise.muscleGroup === muscleFilter);
    if (patternFilter !== 'Todos')
      list = list.filter((exercise) => exercise.movementPattern === patternFilter);
    ...
    <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur flex items-end md:items-center justify-center p-4">
      <div className="bg-brand-gray border-2 border-brand-light/10 w-full max-w-3xl max-h-[90vh] flex flex-col shadow-brutal-neon">
        <div className="flex items-center justify-between p-5 border-b-2 border-brand-light/10">
          <h2 className="font-display text-3xl uppercase tracking-widest text-brand-light">
            Biblioteca de Exercícios
          </h2>
          <button
            type="button"
            onClick={onClose}
            className="p-2 bg-brand-dark border-2 border-brand-light/10 text-brand-light hover:text-brand-neon"
          >
            <X size={18} />
          </button>
        </div>
        ...
        {showCustomForm && (
          <div className="p-4 border-b-2 border-brand-light/10 bg-brand-dark/50 space-y-3">
            ...
            <input
              ...
              onChange={(event) =>
                setNewExercise((value) => ({ ...value, name: event.target.value }))
              }
              className="w-full bg-brand-dark border-2 border-brand-light/10 px-3 py-2 text-sm text-brand-light outline-none focus:border-brand-neon"
            />
            <input
              placeholder="Grupo muscular"
              value={newExercise.muscleGroup}
              onChange={(event) =>
                setNewExercise((value) => ({ ...value, muscleGroup: event.target.value }))
              }
              className="w-full bg-brand-dark border-2 border-brand-light/10 px-3 py-2 text-sm text-brand-light outline-none focus:border-brand-neon"
            />
            <input
              ...
              onChange={(event) =>
                setNewExercise((value) => ({ ...value, movementPattern: event.target.value }))
              }
              className="w-full bg-brand-dark border-2 border-brand-light/10 px-3 py-2 text-sm text-brand-light outline-none focus:border-brand-neon"
            />
            <input
              placeholder="Tags (vírgula separado)"
              value={newExercise.tags}
              onChange={(event) =>
                setNewExercise((value) => ({ ...value, tags: event.target.value }))
              }
              className="w-full bg-brand-dark border-2 border-brand-light/10 px-3 py-2 text-sm text-brand-light outline-none focus:border-brand-neon"
            />
            <button
              ...
              className="bg-brand-neon text-brand-dark px-5 py-2 border-brutal font-bold text-sm uppercase"
            >
              Salvar
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2">
          {filtered.map((exercise) => (
            <div
              key={exercise.id}
              className="flex items-center justify-between gap-3 p-3 border-2 border-brand-light/10 bg-brand-dark hover:border-brand-neon/40 transition-all"
            >
              <div className="min-w-0">
                <p className="text-brand-light font-semibold text-sm">{exercise.name}</p>
                <p className="text-brand-muted text-xs mt-0.5">
                  {exercise.muscleGroup} · {exercise.movementPattern}
                </p>
                <div className="flex gap-1 mt-1 flex-wrap">
                  {exercise.tags.slice(0, 4).map((tag) => (
                    <span
                      key={tag}
                      className="text-[10px] text-brand-light/40 border border-brand-light/10 px-2 py-0.5"
                    >
                      {tag}
                    </span>
                  ))}
                  {exercise.isCustom && (
                    <span className="text-[10px] text-brand-neon border border-brand-neon/30 px-2 py-0.5">
                      custom
                    </span>
                  )}
                </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                {exercise.videoUrl && (
                  <a
                    ...
                    className="text-brand-neon text-xs hover:underline"
                  >
                    vídeo
                  </a>
                )}
          ...
          {filtered.length === 0 && (
            <div className="text-center text-brand-muted py-10 font-mono">
