...
export function InlineNotice({ type = 'info', title, children }: InlineNoticeProps) {
  ...
  return (
    ...
    >
      ...
      <div>
        {title && (
          <h4 className="mb-1 font-mono text-sm font-bold uppercase tracking-wide">{title}</h4>
        )}
        <div className="font-mono text-xs leading-relaxed text-brand-light/80">{children}</div>
      </div>
    </div>
  );
}
