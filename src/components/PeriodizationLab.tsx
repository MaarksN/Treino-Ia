    ...
    <section className="space-y-6">
      <header>
        ...
        <p className="text-brand-muted mt-2 max-w-3xl">
          MEV, MAV, MRV, fadiga acumulada, prontidão de sessão, progressão automática, deload e
        ...
        {phaseSummary.map((item) => {
          ...
          return (
            <div key={item.title} className="bg-brand-gray rounded-3xl border border-white/10 p-5">
              ...
              <p className="text-xs text-brand-muted uppercase tracking-widest">Item {item.item}</p>
        ...
        {performances.length > 0 ? (
          ...
          <div className="bg-brand-gray rounded-3xl border border-white/10 p-8 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-sm text-brand-muted max-w-sm">
              Registre seus treinos para visualizar seu volume em relação ao MEV, MAV e MRV por
              grupo muscular.
            </p>
          </div>
        )}
      ...
      {performances.length > 0 ? (
        <AutoProgressionPanel performances={performances} fatigueScore={fatigueScore} />
      ) : (
        <div className="bg-brand-gray rounded-3xl border border-white/10 p-8 flex flex-col items-center justify-center text-center">
          <Target className="text-brand-muted mb-4" size={40} />
          <h3 className="text-lg font-black text-white mb-2">Sem Dados de Progressão</h3>
          <p className="text-sm text-brand-muted max-w-sm">
            Realize exercícios e avalie o RPE para que o sistema possa sugerir progressões de carga
            e trocas de exercícios automaticamente.
          </p>
        </div>
      )}
      ...
      <section className="bg-brand-gray rounded-3xl border border-white/10 p-5">
        <div className="flex items-center gap-2 mb-5">
          ...
          <div className="flex-1">
            <h2 className="text-xl font-black text-white">Plano Automatizado de 12 Semanas</h2>
        ...
        {isLoadingPlan ? (
          ...
          <div className="bg-brand-dark rounded-2xl border border-white/10 p-8 flex flex-col items-center justify-center text-center">
            ...
            <p className="text-brand-muted mb-6 max-w-md">
              Gere seu primeiro plano automatizado de 12 semanas. Ele será ajustado com base na sua
              fadiga e performance ao longo do tempo.
            </p>
            <button
          ...
          <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-3">
            {planData.plan_data.weeks.map((week) => (
              <div
                key={week.week}
                className={`rounded-2xl border p-4 ${planData.current_week === week.week ? 'bg-brand-neon/10 border-brand-neon/30' : 'bg-brand-dark border-white/10'}`}
              >
