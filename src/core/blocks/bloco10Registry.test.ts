...
import { assertAuthenticatedUser, assertServerSourceOfTruth, bloco10Items, summarizeBloco10 } from './bloco10Registry';
describe('Bloco 10 — Gamificação Profunda Retenção', () => {
  it('mapeia 20 itens', () => { expect(bloco10Items).toHaveLength(20); });
  it('não fica production-ready com itens pendentes', () => { expect(summarizeBloco10().readyForProduction).toBe(false); });
