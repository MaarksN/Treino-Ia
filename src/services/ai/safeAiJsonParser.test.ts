...
describe('safeAiJsonParser', () => {
  const guard = (value: unknown): value is { name: string } =>
    Boolean(value) &&
    typeof value === 'object' &&
  ...
  it('accepts extra fields', () =>
    expect(safeAiJsonParser('{"name":"ok","x":1}', guard).ok).toBe(true));
  it('extracts json from wrappers', () =>
    expect(safeAiJsonParser('text {"name":"ok"} tail', guard).ok).toBe(true));
