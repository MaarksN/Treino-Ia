  ...
  try {
    ...
    if (parsed.ok && parsed.value) {
      return {
        ...
        audit: createAudit(
          ...
          false,
          'valid',
          'Resposta IA validada contra contrato estruturado.',
        ),
      };
    }
    ...
    return {
      ...
      audit: createAudit(
        ...
        true,
        parsed.reason,
        'Resposta IA inválida; fallback determinístico aplicado.',
      ),
    };
  } catch (error) {
    return {
...
function fallbackGeneratedWorkout(
  profile: UserProfile,
  ...
  return {
    ...
    weeklyVolume: flags.lowRecovery
      ? 'Volume reduzido até recuperação normalizar.'
      : 'Volume moderado, ajustado por resposta semanal.',
    idealFrequency: flags.recommendedFrequency,
    safetyNotes: flags.painOrLimitation
      ? ['Evitar padrões que agravem dor ou limitação informada.']
      : ['Manter RPE controlado nas primeiras semanas.'],
    initialProgression:
      'Aumentar carga somente quando reps alvo forem concluídas com técnica estável.',
    summary:
      'Plano estruturado por regras locais enquanto a resposta IA não estiver disponível ou validada.',
  };
}
...
function fallbackWeeklyAdaptation(flags: DeterministicFlags): WeeklyPlanAdaptation {
  return enforceTrainingGuardrails(
    {
      changes: flags.deloadNeeded
        ? ['Reduzir volume da próxima semana e remover técnicas intensificadoras.']
        : ['Manter progressão gradual e monitorar RPE.'],
      ...
      safetyNotes: [],
      summary: flags.deloadNeeded
        ? 'Adaptação conservadora por sinais de fadiga/recuperação.'
        : 'Adaptação conservadora por contrato local.',
    },
    flags,
  );
}

function fallbackLoadProgression(
  plan: WorkoutPlan,
  flags: DeterministicFlags,
): LoadProgressionAdvice {
  const action = flags.lowRecovery || flags.deloadNeeded ? 'reduce' : 'maintain';
  return {
    items: flattenExercises(plan)
      .slice(0, 8)
      .map((exercise) => ({
        exercise,
        action,
        adjustment:
          action === 'reduce'
            ? 'Reduzir 5-10% ou 1 serie.'
            : 'Manter carga ate completar reps alvo com RPE controlado.',
        reason: flags.deloadNeeded
          ? 'Fadiga/recuperação pedem progressão conservadora.'
          : 'Sem evidência validada para aumento agressivo.',
      })),
...
function fallbackPlateau(plan: WorkoutPlan, flags: DeterministicFlags): PlateauPrediction {
  return {
    ...
    probableCause:
      flags.plateauRisk === 'alto'
        ? 'Histórico recente com pouca variação de volume.'
        : 'Sem platô determinístico forte.',
    suggestedAction:
      flags.plateauRisk === 'alto'
        ? 'Aplicar microvariação de volume, repetições ou exercício por 1-2 semanas.'
        : 'Continuar monitorando carga, reps e RPE.',
...
function fallbackDeload(flags: DeterministicFlags): DeloadAdvice {
  return {
    ...
    nextWeekAdjustment: flags.deloadNeeded
      ? 'Reduzir volume e intensidade por 5-7 dias.'
      : 'Manter treino e reavaliar sinais na próxima sessão.',
    volumeReductionPercent: flags.deloadNeeded ? 35 : 0,
    intensityReductionPercent: flags.deloadNeeded ? 10 : 0,
    summary: flags.deloadNeeded
      ? 'Deload recomendado por guardrail determinístico.'
      : 'Deload não obrigatório pelos dados locais.',
  };
}

function fallbackExerciseAlternatives(
  exerciseName: string,
  equipment: string,
  injuries: string,
): ExerciseAlternativesAdvice {
  const hasLimitation =
    ...
    !['nenhuma', 'sem', 'nao', 'não'].includes(injuries.trim().toLowerCase());
  return {
    byEquipment: [
      {
        name: `${exerciseName} com equipamento disponível`,
        reason: `Adaptado para ${equipment || 'peso corporal/equipamento informado'}.`,
        safetyNote: 'Manter amplitude sem dor.',
      },
      {
        name: `Variação unilateral de ${exerciseName}`,
        reason: 'Permite menor carga absoluta e mais controle.',
        safetyNote: 'Usar cadência controlada.',
      },
    ],
    byPainOrLimitation: hasLimitation
      ? [
          {
            name: `Alternativa sem dor para ${exerciseName}`,
            reason: `Limitação informada: ${injuries}.`,
            safetyNote: 'Interromper se houver dor aguda ou irradiação.',
          },
        ]
      : [],
    summary: hasLimitation
      ? 'Substituição segura priorizada por dor/limitação.'
      : 'Substituições seguras por equipamento.',
  };
}
  ...
  return {
    methods: advanced
      ? [
          {
            name: 'Rest-pause',
            applyTo: 'Exercícios acessórios estáveis.',
            caution: 'Evitar em semanas de fadiga alta.',
          },
          {
            name: 'Drop set',
            applyTo: 'Última série de isoladores.',
            caution: 'Não usar em movimentos com dor.',
          },
        ]
      : [
          {
            name: 'Progressão dupla',
            applyTo: 'Exercícios principais.',
            caution: 'Priorizar técnica antes de intensidade avançada.',
          },
        ],
    summary: advanced
      ? 'Métodos avançados liberados com cautela.'
      : 'Métodos conservadores por nível de experiência.',
  };
}

function fallbackIdealFrequency(
  profile: UserProfile,
  flags: DeterministicFlags,
): IdealFrequencyRecommendation {
  return {
    ...
    recoveryNotes: flags.deloadNeeded
      ? ['Reduzir uma sessão nesta semana ou trocar por recuperação ativa.']
      : ['Reavaliar semanalmente por sono, estresse e RPE.'],
...
function fallbackSleepStress(sleepHours: string, stressLevel: string): SleepStressAdjustment {
  ...
  const poor =
    (Number.isFinite(sleep) && sleep < 6) ||
    /alto|alta|ruim/i.test(stressLevel) ||
  ...
  return {
    ...
    notes: poor
      ? ['Sono/estresse ruins: reduzir agressividade do treino hoje.']
      : ['Sono/estresse sem alerta crítico informado.'],
    summary: poor
      ? 'Ajuste conservador aplicado por guardrail de sono/estresse.'
      : 'Treino mantido com monitoramento normal.',
  };
}
...
function fallbackWeeklyInsights(flags: DeterministicFlags): WeeklyInsightsContract {
  return {
    consistency:
      flags.adherenceRisk === 'alto'
        ? 'Consistência baixa nos registros recentes.'
        : 'Consistência sem alerta crítico.',
    fatigue: flags.deloadNeeded
      ? 'Fadiga/recuperação pedem redução temporária.'
      : 'Fadiga sem sinal crítico.',
    progress:
      flags.plateauRisk === 'alto'
        ? 'Possível platô no histórico recente.'
        : 'Progressão deve seguir controlada.',
    adherence: flags.adherenceRisk,
    nextWeekRecommendation: flags.deloadNeeded
      ? 'Semana conservadora com menor volume.'
      : 'Manter plano e registrar sessões.',
...
function fallbackAbandonmentRisk(flags: DeterministicFlags): AbandonmentRiskContract {
  return {
    ...
    signals:
      flags.adherenceRisk === 'alto'
        ? ['Baixo número de sessões recentes registradas.']
        : ['Sem alerta alto de abandono.'],
    recommendedAction:
      flags.adherenceRisk === 'alto'
        ? 'Reduzir fricção: treino mais curto e meta mínima semanal.'
...
function dayVariationFallback(
  ...
  return {
    ...guardrails,
    variations: flattenExercises(plan)
      .slice(0, 4)
      .map((exercise) => `${exercise}: manter padrão, reduzir séries se necessário.`),
...
SESSOES: ${JSON.stringify(sessions.slice(-10))}`,
    ...
    listDeterministicFlagLabels(flags),
    (value) =>
      flags.lowRecovery || flags.deloadNeeded
        ? {
            ...value,
            items: value.items.map((item) => ({
              ...item,
              action: item.action === 'increase' ? 'maintain' : item.action,
            })),
          }
        : value,
  );
...
SESSOES: ${JSON.stringify(sessions.slice(-12))}`,
    ...
    (value) =>
      flags.plateauRisk === 'alto'
        ? { ...value, risk: 'alto' as const, plateauDetected: true }
        : value,
  );
}
    ...
    (value) =>
      flags.deloadNeeded
        ? {
            ...value,
            needed: true,
            volumeReductionPercent: Math.max(value.volumeReductionPercent, 30),
          }
        : value,
  );
}
...
export async function generateMicrocycles(
  profile: UserProfile,
  goal: string,
): Promise<Microcycle[]> {
  const response = await getAI().models.generateContent({
...
export async function adjustBySleepAndStress(
  plan: WorkoutPlan,
  sleepHours: string,
  stressLevel: string,
) {
  ...
  const flagLabels =
    fallback.aggressiveness === 'conservative'
      ? ['poor_sleep_or_stress_reduce_aggressiveness']
      : [];
  return generateStructured(
...
PLANO: ${JSON.stringify(plan)}`,
    ...
    (value) =>
      fallback.aggressiveness === 'conservative'
        ? {
            ...value,
            intensityAdjustment: 'reduce' as const,
            volumeAdjustment: 'reduce' as const,
    ...
    (value) =>
      flags.adherenceRisk === 'alto'
        ? {
            ...value,
            risk: 'alto' as const,
            alert: value.alert || 'ALERTA: risco alto de abandono.',
          }
        : value,
  );
}

export async function generateDayVariations(
  plan: WorkoutPlan,
  context: string,
  recovery?: RecoveryCheckin,
) {
...
PLANO: ${JSON.stringify(plan)}`,
    ...
    (value) => ({
      ...
      intensityAdjustment:
        fallback.intensityAdjustment === 'reduce' ? 'reduce' : value.intensityAdjustment,
