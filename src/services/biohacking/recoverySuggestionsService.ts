...
export const generateRecoverySuggestion = (metrics: PostWorkoutMetrics): RecoverySuggestion => {
  const disclaimer =
    'Aviso: Esta é uma sugestão de bem-estar, não uma recomendação médica. Consulte um profissional de saúde se sentir dor aguda.';
  ...
  if (metrics.muscleSoreness > 6) {
    return {
      ...
      description:
        'Dor muscular moderada/alta. Considere recuperação ativa leve ou alongamento suave.',
      disclaimer,
    };
  }
