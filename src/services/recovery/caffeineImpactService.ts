...
export function estimateCaffeineImpact(
  entries: CaffeineEntry[],
  sleepHour = 23,
): { totalMg: number; nearSleepMg: number; message: string } {
  ...
  if (!entries.length)
    return { totalMg, nearSleepMg, message: 'Sem consumo de cafeína registrado hoje.' };
  if (nearSleepMg >= 120)
    return {
      totalMg,
      nearSleepMg,
      message: 'Cafeína próxima do sono pode atrapalhar sua recuperação hoje.',
    };
  if (totalMg >= 350)
    return {
      totalMg,
      nearSleepMg,
      message: 'Consumo alto de cafeína hoje. Considere reduzir no fim do dia.',
    };
