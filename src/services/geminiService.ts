...
function isBodyweightProfile(profile: UserProfile) {
  const text =
    `${profile.workoutLocation || ''} ${profile.gymType || ''} ${profile.equipment || ''}`.toLowerCase();
...
function getTrainingParams(profile: UserProfile) {
  ...
  const time = Number(
    profile.timePerWorkout || String(profile.sessionDuration || '').match(/\d+/)?.[0] || 60,
  );
  ...
  if (
    goal.includes('emagrecimento') ||
    goal.includes('resistência') ||
    goal.includes('condicionamento')
  ) {
    return {
...
function localExerciseFromTuple(
  ...
  const reps = isCoreOrConditioning
    ? movementPattern === 'Condicionamento'
      ? '30-45s'
      : '30-60s'
    : params.reps;
  ...
  return {
    ...
    name,
    sets:
      index === 0 && !profile.experienceLevel.toLowerCase().includes('iniciante')
        ? params.sets + 1
        : params.sets,
    reps,
...
function pickLocalExercises(
  ...
  const bank = isBodyweightProfile(profile)
    ? LOCAL_EXERCISE_BANK.bodyweight
    : LOCAL_EXERCISE_BANK.gym;
...
function getSplit(daysPerWeek: number) {
  const splits: Record<
    number,
    Array<{ name: string; focus: string; groups: Array<'push' | 'pull' | 'legs' | 'core'> }>
  > = {
    1: [{ name: 'Dia 1', focus: 'Full Body Essencial', groups: ['legs', 'push', 'pull', 'core'] }],
    2: [
...
function generateLocalWorkoutPlan(profile: UserProfile, history?: WorkoutPlan[]): WorkoutPlan {
  ...
  return {
    ...
    days: split.map((day, dayIndex) => {
      const exercises = pickLocalExercises(profile, day.groups, params.exerciseCount).map(
        (tuple, index) => localExerciseFromTuple(tuple, params, index, profile),
      );
      ...
      return {
        ...
        focus: day.focus,
        warmup:
          '5-8 minutos de aquecimento geral, mobilidade da articulação principal do dia e 2 séries leves do primeiro exercício.',
        cooldown:
          '3-5 minutos de respiração, alongamento leve dos músculos treinados e anotação de carga/repetições.',
        estimatedDuration: profile.sessionDuration || `${profile.timePerWorkout || 60} minutos`,
        exercises:
          dayIndex === split.length - 1 && weakPoint
            ? exercises.map((exercise, index) =>
                index === exercises.length - 1
                  ? {
                      ...exercise,
                      notes: `${exercise.notes} Finalize com foco extra no ponto fraco informado: ${weakPoint}.`,
                    }
                  : exercise,
              )
            : exercises,
            ...
            items: {
              ...
              properties: {
                ...
                concentricPhase: {
                  type: Type.STRING,
                  description:
                    'Explicação da fase concêntrica (explosão/contração) deste exercício e músculos envolvidos.',
                },
                eccentricPhase: {
                  type: Type.STRING,
                  description:
                    'Explicação da fase excêntrica (alongamento/controle) deste exercício e músculos envolvidos.',
                },
                notes: {
                  type: Type.STRING,
  ...
  if (history && history.length > 0) {
    ...
    if (feedbacks.length > 0) {
      feedbackContext =
        ...
        feedbacks
          .map((f) => {
            if (f.feedback === 'painful')
              return `- ${f.name}: Relatou dor (substitua por outro ou mude a mecânica).`;
            if (f.feedback === 'easy')
              return `- ${f.name}: Achou muito fácil (aumente a dificuldade/carga recomendada).`;
            if (f.feedback === 'hard')
              return `- ${f.name}: Achou muito difícil (reduza a dificuldade/carga recomendada).`;
            return '';
          })
          .filter(Boolean)
          .join('\n');
...
export async function extractWorkoutFromFile(
  base64Data: string,
  mimeType: string,
): Promise<WorkoutPlan> {
...
export async function suggestExerciseVariations(
  ...
  Array<{ name: string; description: string; difficulty: 'Easier' | 'Harder' | 'Different Focus' }>
> {
...
${
  userProfile
    ? `Perfil do usuário (para adaptar as sugestões):
...
- Restrições/Lesões: ${userProfile.injuries || 'Nenhuma'}`
  ...
  const jsonStr = response.text || '[]';
  try {
    const parsedResult = safeAiJsonParser<
      Array<{
        name: string;
        description: string;
        difficulty: 'Easier' | 'Harder' | 'Different Focus';
      }>
    >(
      jsonStr,
      (
        ...
        value.every(
          (item) =>
            ...
            ['Easier', 'Harder', 'Different Focus'].includes(
              String((item as { difficulty?: unknown }).difficulty),
            ),
        ),
    );
...
export async function adjustWorkoutForRecovery(
  plan: WorkoutPlan,
...
export async function generateQuickWorkout(
  ...
  const constraints: Record<string, string> = {
    express:
      'Treino completo em 15-20 minutos. Máximo 5 exercícios, descanso curto e foco em eficiência.',
