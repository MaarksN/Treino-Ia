...
export function estimateMacroTargetsForLifestyleProfile(
  profile: LifestyleMacroProfile,
): MacroTargets {
  ...
  const calories =
    goal.includes('emagrec') || goal.includes('cut')
...
export function getDynamicNutritionRecommendation(
  input: DynamicNutritionInput,
): DynamicNutritionRecommendation {
  ...
  const level: FatigueLevel =
    fatigueScore >= 65 ? 'alto' : fatigueScore >= 38 ? 'moderado' : 'baixo';
  ...
  const flags = [
    ...
    ...(input.menstrualPhase === 'menstrual' || input.menstrualPhase === 'lútea'
      ? [`fase_${input.menstrualPhase}`]
      : []),
  ];
...
export function summarizeMealScanForTargets(
  ...
  const verdict: MealScanInsight['verdict'] =
    proteinDeltaG >= -5 && Math.abs(calorieDelta) <= 180
      ? 'excelente'
