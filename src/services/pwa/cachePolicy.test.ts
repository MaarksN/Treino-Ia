...
describe('service worker cache policy', () => {
  ...
  it('does not cache /api/gamification/event', () => {
    expect(getServiceWorkerCacheStrategy({ url: '/api/gamification/event' }, APP_ORIGIN)).toBe(
      'network-only',
    );
  });

  it('allows static assets to use cache-first', () => {
    expect(getServiceWorkerCacheStrategy({ url: '/assets/app.js' }, APP_ORIGIN)).toBe(
      'cache-first',
    );
  });
