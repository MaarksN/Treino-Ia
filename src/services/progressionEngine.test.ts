...
type CompletedExercise = WorkoutSession['exercises'][number];

function buildWorkoutSession(exercise: Partial<CompletedExercise> = {}): WorkoutSession {
  return {
    id: '1',
    planId: '1',
    dayId: '1',
...
describe('progressionEngine', () => {
  ...
  it('sugere increase quando completou séries e reps consistentemente', () => {
    const history = [buildWorkoutSession()];
  ...
  it('sugere maintain quando performance é mediana (alto RPE)', () => {
    const history = [buildWorkoutSession({ sets: [{ reps: 10, rpe: 9.5, weight: 100 }] })];
  ...
  it('sugere decrease quando há falhas repetidas', () => {
    const history = [buildWorkoutSession({ sets: [{ reps: 5, rpe: 10, weight: 100 }] })];
  ...
  it('não quebra com campos opcionais ausentes', () => {
    const history = [buildWorkoutSession({ actualWeight: undefined, sets: undefined })];
