...
export function useWorkoutManager({
  ...
  const refreshEngagement = useCallback(
    (nextStreak = streakData, nextHistory = analyticsHistory, nextCheckins = allCheckins) => {
  ...
  const saveLocalDashboardSnapshot = useCallback(
    (nextHistory = analyticsHistory, nextStreak = streakData, nextCheckins = allCheckins) => {
      try {
  ...
  const handleCompleteDay = useCallback(
    (record: WorkoutHistoryRecord) => {
      ...
      void persistWorkoutHistoryToBackend(newHistory).catch((error) =>
        captureError(error, 'App.persistWorkoutHistory'),
      );
      enqueueWorkoutSync(record);
      void recordGamificationEvent('workout_completed', record.id).catch((error) =>
        captureError(error, 'App.workoutCompletedGamification'),
      );

      const completedPlan = plans.find((plan) => plan.id === record.planId);
      const completedDayIndex =
        completedPlan?.days.findIndex((day) => day.id === record.dayId) ?? -1;
