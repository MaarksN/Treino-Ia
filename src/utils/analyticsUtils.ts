...
export function recordWorkoutSession(
  ...
  const prsBroken = exercises
    .filter(
      (exercise) =>
        exercise.setLogs?.some((log) => log.weight && log.reps) || exercise.actualWeight,
    )
  ...
  for (let weekOffset = weeksBack - 1; weekOffset >= 0; weekOffset--) {
    ...
    weeks.push({
      ...
      avgReadiness: weekEntries.length
        ? weekEntries.reduce((sum, entry) => sum + (entry.readinessScore || 0), 0) /
          weekEntries.length
        : 0,
...
export function getWeeklyMuscleGroupVolume(
  records: WorkoutHistoryRecord[],
  weeksBack = 8,
): WeeklyMuscleGroupVolume[] {
  const weeks = Array.from({ length: weeksBack }, (_, index) =>
    getWeekWindow(weeksBack - 1 - index),
  );
...
export function getWeeklyAverageRpe(
  records: WorkoutHistoryRecord[],
  weeksBack = 8,
): WeeklyRpeTrend[] {
  const weeks = Array.from({ length: weeksBack }, (_, index) =>
...
export function getWeekOverWeekComparison(
  ...
  const summarize = (window: ReturnType<typeof getWeekWindow>) => {
    ...
    const rpeValues = recordsFor(window).flatMap((record) =>
      record.exercises.flatMap(getExerciseRpeValues),
    );
...
export function getGoalProgressIndicators(
  ...
  const volumeDirection =
    comparison.volumeDelta > 0 ? 'subiu' : comparison.volumeDelta < 0 ? 'caiu' : 'manteve';
    ...
    return [
      {
        ...
        detail:
          adherencePercent >= 80
            ? 'Frequência forte para sustentar déficit sem perder treino.'
            : 'A frequência ainda limita o gasto semanal planejado.',
    ...
    return [
      {
        ...
        detail:
          comparison.volumeDelta >= 0
            ? 'Carga semanal em progressão.'
            : 'Carga semanal caiu; verifique fadiga ou faltas.',
        tone: comparison.volumeDelta >= 0 ? 'good' : 'warning',
      },
      {
        label: 'RPE médio',
        ...
        detail:
          comparison.current.avgRpe >= 8.8
            ? 'Intensidade alta; monitore deload.'
            : 'Intensidade controlada para acumular prática.',
        tone:
          comparison.current.avgRpe >= 9
            ? 'danger'
            : comparison.current.avgRpe >= 8
              ? 'warning'
              : 'info',
      },
    ];
  ...
  return [
    {
      ...
      detail:
        comparison.volumeDelta >= 0
          ? 'Volume semanal sustentado ou crescente.'
          : 'Volume caiu; pode reduzir estímulo de recomposição.',
      tone: comparison.volumeDelta >= 0 ? 'good' : 'warning',
    },
    {
      ...
      detail:
        adherencePercent >= 75
          ? 'Boa base para evolução contínua.'
          : 'Ajuste agenda ou número de sessões esperadas.',
...
export function getAvgSessionDuration(history: WorkoutHistoryEntry[]): number {
  ...
  return Math.round(
    withDuration.reduce((sum, entry) => sum + (entry.durationMinutes || 0), 0) /
      withDuration.length,
  );
}
