...
export function scoreNutritionDay(
  ...
  const calorieDelta =
    Math.abs(consumed.calories - targets.calories) / Math.max(1, targets.calories);
  const proteinDelta =
    Math.abs(consumed.proteinG - targets.proteinG) / Math.max(1, targets.proteinG);
  const waterDelta =
    consumed.waterMl >= targets.waterMl
      ? 0
      : (targets.waterMl - consumed.waterMl) / targets.waterMl;
  ...
  if (score >= 60)
    return { score, grade: 'D', tip: 'Planeje refeicoes rapidas para evitar buracos.' };
  if (score >= 50)
    return { score, grade: 'E', tip: 'Reforce o basico: proteina, agua e calorias.' };
