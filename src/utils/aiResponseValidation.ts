...
export function isGeneratedWorkoutFromAnamnesis(
  value: unknown,
): value is GeneratedWorkoutFromAnamnesis {
  return (
    isRecord(value) &&
...
export function isExerciseAlternativesAdvice(value: unknown): value is ExerciseAlternativesAdvice {
  const isAlternative = (item: unknown) =>
    isRecord(item) && isString(item.name) && isString(item.reason) && isString(item.safetyNote);
...
export function isAdvancedMethodsRecommendation(
  value: unknown,
): value is AdvancedMethodsRecommendation {
  return (
    isRecord(value) &&
    Array.isArray(value.methods) &&
    value.methods.every(
      (item) =>
        isRecord(item) && isString(item.name) && isString(item.applyTo) && isString(item.caution),
    ) &&
    isString(value.summary)
  );
}

export function isIdealFrequencyRecommendation(
  value: unknown,
): value is IdealFrequencyRecommendation {
  return (
    isRecord(value) &&
