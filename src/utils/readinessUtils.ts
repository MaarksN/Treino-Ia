...
export function calculateReadiness(checkin: DailyCheckin): ReadinessScore {
  ...
  const total = Math.max(
    0,
    Math.min(100, Math.round(sleepScore + energyScore + stressScore + sorenessScore)),
  );
  ...
  if (total >= 80) {
    return {
      ...
      label: 'Excelente',
      color: '#a3e635',
      recommendation: 'Treino completo',
  ...
  if (total >= 35) {
    return {
      ...
      label: 'Baixa',
      color: '#fb923c',
      recommendation: 'Treino leve',
      adjustedIntensity: 50,
    };
  }
