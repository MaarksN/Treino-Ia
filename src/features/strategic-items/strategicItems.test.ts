...
describe('strategicItemsRegistry', () => {
  ...
  it('tracks batch 02 gamification and retention items only', () => {
    ...
    expect(
      batchItems.filter((item) => item.status === 'implemented_now').map((item) => item.id),
    ).toEqual(batchIds);
  });
  ...
  it('tracks batch 06 UI accessibility and interaction items only', () => {
    ...
    expect(
      batchItems.filter((item) => item.status === 'implemented_now').map((item) => item.id),
    ).toEqual([13, 14, 15, 18, 19]);
    expect(
      batchItems.filter((item) => item.status === 'foundation_created').map((item) => item.id),
    ).toEqual([]);
  });
