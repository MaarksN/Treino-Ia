...
describe('sanitizeRedirectTarget', () => {
  ...
  it('accepts valid relative redirects', () => {
    expect(sanitizeRedirectTarget('/dashboard?tab=health', BASE_URL)).toBe(
      `${BASE_URL}/dashboard?tab=health`,
    );
  });
  ...
  it('accepts allowlisted origin redirects', () => {
    ...
    expect(sanitizeRedirectTarget('https://preview.treino.ai/health', BASE_URL)).toBe(
      'https://preview.treino.ai/health',
    );
  });
