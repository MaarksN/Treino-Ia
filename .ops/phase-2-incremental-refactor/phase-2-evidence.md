...
## Ajuste para falha rápida nos jobs CI/Lighthouse (GitHub Actions)
- Ajuste mínimo aplicado nos workflows: `node-version` alterado de `24` para `22` em `ci.yml` e `lighthouse.yml` para alinhar com LTS estável e reduzir risco de falha precoce no setup do runtime.
- Escopo preservado: nenhuma mudança de código da aplicação, thresholds, Lighthouse config ou comportamento de produção.
